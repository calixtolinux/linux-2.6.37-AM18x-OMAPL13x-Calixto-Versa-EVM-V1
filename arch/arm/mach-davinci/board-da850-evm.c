...
static __init void da850_evm_init(void)
{
	...
	ret = da8xx_register_lcdc(&vga_monitor_pdata);
        if (ret)
                pr_warning(KERN_WARNING"da850_evm_init: VGA registration failed: %d\n", ret);

        printk(KERN_ERR"da850_evm_init: VGA registration Done: %d\n", ret);
        ...
        printk(KERN_ERR"calixto-evm_da850_evm_init: LCD registration Done: %d\n", ret);

        /* End Calixto-lcd_evm registration */
